...
import {
  ...
  driverGradientFromName,
} from '../utils/driver-display';
...
export function DriverCreativeAvatar({
  name,
  imageUrl,
  size = 'sm',
  className,
  ...
  const resolved = resolveDriverImageUrl(imageUrl ?? undefined);
  const showImage = Boolean(resolved) && !broken;
  const g = driverGradientFromName(name);
