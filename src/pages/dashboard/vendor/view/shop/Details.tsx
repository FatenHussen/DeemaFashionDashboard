    ...
    return (
      <Box className="flex items-center justify-center min-h-[400px] p-6">
        <Box className="w-full max-w-md rounded-xl border border-border/50 shadow-lg bg-background p-6">
          <Typography variant="h6" className="text-destructive mb-2">{t('shopDetails.errorTitle')}</Typography>
          <Typography variant="body2" className="text-muted-foreground mb-4">
            {error instanceof Error ? error.message : t('shopDetails.errorBody')}
          </Typography>
          <Button variant="outlined" onClick={() => navigate('/shop')}>{t('shopDetails.backToList')}</Button>
        </Box>
      </Box>
    );
  }
        ...
        <Box className="relative w-full">
          <Box className="mb-6">
            <Button variant="text" onClick={() => navigate('/shop')} className="mb-4 -ml-2 text-muted-foreground hover:text-foreground">
              <Iconify icon="solar:arrow-left-bold" width={20} className="mr-2" /> {t('shopDetails.backToList')}
            </Button>
            <Box className="flex items-center gap-4 mb-2">
              ...
              <Box className="flex items-center gap-2">
                <span className={`inline-flex items-center rounded-full px-2.5 py-1 text-xs font-medium ${item.is_open_now ? 'bg-emerald-500/20 text-emerald-600' : 'bg-muted text-muted-foreground'}`}>
                  {item.is_open_now ? t('columns.openNow') : t('closed')}
                </span>
          ...
          <Box className="rounded-2xl border border-border/50 bg-card/80 backdrop-blur-sm shadow-sm overflow-hidden mb-4">
            <Box className="p-6">
              <Typography variant="h6" className="font-semibold mb-4">{t('shopDetails.information')}</Typography>
              <Box className="grid gap-4 sm:grid-cols-2">
                <Box>
                  <Typography variant="caption" className="text-muted-foreground">{t('columns.vendor')}</Typography>
                ...
                <Box>
                  ...
                  <Box className="flex items-center gap-1">
                    ...
                    <Typography variant="body1" className="font-medium">
                      {item.average_rating?.toFixed(1) ?? '0.0'} ({t('shopDetails.reviewsCount', { count: item.ratings_count ?? 0 })})
                    </Typography>
                  </Box>
                </Box>
                <Box>
                  <Typography variant="caption" className="text-muted-foreground">{t('columns.email')}</Typography>
                  <Typography variant="body1" className="font-medium">{item.email || '-'}</Typography>
                </Box>
                <Box>
                  <Typography variant="caption" className="text-muted-foreground">{t('columns.phone')}</Typography>
                  <Typography variant="body1" className="font-medium">{item.phone || '-'}</Typography>
                </Box>
                <Box>
                  <Typography variant="caption" className="text-muted-foreground">{t('columns.mobile')}</Typography>
                  <Typography variant="body1" className="font-medium">{item.mobile || '-'}</Typography>
                </Box>
                <Box>
                  <Typography variant="caption" className="text-muted-foreground">{t('columns.address')}</Typography>
                  <Typography variant="body1" className="font-medium">{formatTranslated(item.address) || '-'}</Typography>
                </Box>
                {item.area && (
                  <>
                    <Box>
                      <Typography variant="caption" className="text-muted-foreground">{t('area')}</Typography>
                      <Typography variant="body1" className="font-medium">
                        {formatTranslated(item.area.name) || '-'}
                      </Typography>
                    </Box>
                    <Box>
                      <Typography variant="caption" className="text-muted-foreground">{t('shopDetails.locationPath')}</Typography>
                      <Typography variant="body1" className="font-medium">
                        {[
                    ...
                    {item.area.base_fee != null && (
                      <Box>
                        <Typography variant="caption" className="text-muted-foreground">{t('form.baseFee')}</Typography>
                ...
                {item.services && item.services.length > 0 && (
                  <Box className="sm:col-span-2">
                    <Typography variant="caption" className="text-muted-foreground">{t('columns.services')}</Typography>
                ...
                {(item.created_at || item.updated_at) && (
                  <Box className="sm:col-span-2 flex gap-6 pt-2 border-t border-border/30">
                    {item.created_at && (
                      <Box>
                        <Typography variant="caption" className="text-muted-foreground">{t('columns.created')}</Typography>
                    ...
                    {item.updated_at && (
                      <Box>
                        <Typography variant="caption" className="text-muted-foreground">{t('columns.updated')}</Typography>
                        <Typography variant="body2" className="font-medium">
          ...
          {item.working_hours && (
            <Box className="rounded-2xl border border-border/50 bg-card/80 backdrop-blur-sm shadow-sm overflow-hidden mb-4">
              <Box className="p-6">
                <Typography variant="h6" className="font-semibold mb-4">{t('shopDetails.workingHours')}</Typography>
                  ...
                  {(['monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday'] as const).map((day) => {
                    ...
                    return (
                      <Box key={day} className="flex items-center justify-between py-1 border-b border-border/30 last:border-0">
                        <Typography variant="body2" className="capitalize font-medium">{t(`weekdays.${day}`)}</Typography>
                        <Typography variant="body2" className="text-muted-foreground">
                          {formatWorkingHoursDay(schedule, t('closed'))}
                        </Typography>
                      </Box>
                    );
                  })}
          ...
          {(item.lat ?? item.lng ?? item.area?.lat ?? item.area?.lng) && (
            <Box className="rounded-2xl border border-border/50 bg-card/80 backdrop-blur-sm shadow-sm overflow-hidden">
              <Box className="p-6">
                <Typography variant="h6" className="font-semibold mb-4">{t('shopDetails.locationCoordinates')}</Typography>
                <Box className="grid gap-4 sm:grid-cols-2">
                  <Box>
                    <Typography variant="caption" className="text-muted-foreground">{t('columns.latitude')}</Typography>
                    <Typography variant="body1" className="font-medium font-mono">
                      {item.lat ?? item.area?.lat ?? '-'}
                    </Typography>
                  </Box>
                  <Box>
                    <Typography variant="caption" className="text-muted-foreground">{t('columns.longitude')}</Typography>
                    <Typography variant="body1" className="font-medium font-mono">
