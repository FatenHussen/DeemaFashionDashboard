...
import { useFetchAffiliateWalletTransactionById } from '@/pages/dashboard/affiliate-wallet-transactions/hooks/affiliate-wallet-transaction';
...
> = {
  commission: {
    gradient:
      'from-emerald-500/[0.18] via-teal-500/[0.08] to-background dark:from-emerald-500/25 dark:via-teal-950/40 dark:to-background',
  ...
  withdraw: {
    gradient:
      'from-amber-500/[0.18] via-orange-500/[0.08] to-background dark:from-amber-500/20 dark:via-orange-950/35 dark:to-background',
...
export default function DetailsPage() {
  ...
  const { data: response, isLoading } = useFetchAffiliateWalletTransactionById(id || '');
    ...
    return (
      <Box className="flex min-h-[50vh] w-full flex-col items-center justify-center gap-4 px-6">
        ...
        <Typography variant="h6" className="text-destructive">
          {t('form.affiliateWalletTxNotFound')}
        </Typography>
        <Button variant="outlined" onClick={() => navigate(paths.dashboard.affiliateWalletTransactions)}>
          {t('form.backToAffiliateWalletTransactions')}
        </Button>
      </Box>
    );
  }
  ...
  const typeLabel = t(`form.affiliateWalletTxType_${item.type}`, { defaultValue: item.type });

  return (
    <>
      <title>{t('form.affiliateWalletTxDetailsDocumentTitle', { appName: CONFIG.appName })}</title>

      <Box className="w-full max-w-[100vw] px-4 pb-12 pt-2 sm:px-6 lg:px-8 xl:px-10">
        <div className="mb-8 flex flex-wrap items-center justify-between gap-4">
          <Button
            variant="text"
            onClick={() => navigate(paths.dashboard.affiliateWalletTransactions)}
              ...
              <div className="mb-6 flex flex-col gap-6 sm:flex-row sm:items-start">
                <div className="relative shrink-0">
                  <div
                    className={cn(
                      ...
                      item.type === 'commission' ? 'bg-emerald-400/40' : 'bg-amber-400/35'
                    )}
                  />
                  <div className="relative rounded-[1.75rem] border border-white/25 bg-background/30 p-1 shadow-2xl backdrop-blur-md dark:border-white/10 dark:bg-black/30">
                    <AffiliateCreativeAvatar
                      ...
                      imageUrl={item.affiliate?.image_url}
                      size="hero"
                      rounded="2xl"
                    />
                  </div>
                ...
                <div className="min-w-0 flex-1">
                  <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-white/20 bg-background/40 px-3 py-1 text-xs font-medium backdrop-blur-md dark:border-white/10 dark:bg-black/20">
                    ...
                    <span className="text-muted-foreground">{t('form.affiliateWalletTxLedgerEntry')}</span>
                  </div>
                  <Typography
                    variant="h3"
                    className="mb-3 text-balance text-2xl font-bold tracking-tight sm:text-3xl lg:text-4xl"
                  >
                    {t('form.affiliateWalletTransactionTitle', { id: item.id })}
                  </Typography>
            ...
            <SectionCard title={t('columns.order')} icon="solar:bag-5-bold">
              {orderHref ? (
                ...
                >
                  <div>
                    <p className="text-xs font-semibold uppercase text-muted-foreground">
                      {t('form.affiliateWalletTxOrderLink')}
                    </p>
                ...
                <div className="rounded-xl border border-dashed border-border/80 bg-muted/15 px-4 py-8 text-center">
                  ...
                  />
                  <p className="text-sm text-muted-foreground">{t('form.affiliateWalletTxNoOrder')}</p>
                </div>
              )}
            </SectionCard>
