...
export default function DetailsPage() {
    ...
    return (
      <Box className="flex items-center justify-center min-h-[400px] p-6">
        <Box className="w-full max-w-md rounded-xl border border-border/50 shadow-lg bg-background p-6">
          <Typography variant="h6" className="text-destructive mb-2">{t('form.giftLoadErrorTitle')}</Typography>
          <Typography variant="body2" className="text-muted-foreground mb-4">{t('form.giftLoadErrorFallback')}</Typography>
          <Button variant="outlined" onClick={() => navigate('/gifts')}>{t('form.backToGifts')}</Button>
        </Box>
      </Box>
    );
  }
  ...
  return (
    <>
      <title>{t('form.giftDetailsDocumentTitle', { appName: CONFIG.appName })}</title>
        ...
        <Box className="relative max-w-4xl mx-auto">
          <Box className="mb-6">
            <Button variant="text" onClick={() => navigate('/gifts')} className="mb-4 -ml-2 text-muted-foreground hover:text-foreground">
              <Iconify icon="solar:arrow-left-bold" width={20} className="mr-2" /> {t('form.backToGifts')}
            </Button>
            <Box className="flex items-center gap-4 mb-2">
              ...
              <Box className="flex-1">
                ...
                <Typography variant="body2" className="text-muted-foreground">
                  {t('form.giftPointsRequiredLine', { points: item.points_required })}
                </Typography>
              </Box>
          ...
          <Box className="rounded-2xl border border-border/50 bg-card/80 backdrop-blur-sm shadow-sm overflow-hidden">
            <Box className="p-6">
              <Typography variant="h6" className="font-semibold mb-4">{t('form.giftInformationSection')}</Typography>
              <Box className="grid gap-4 sm:grid-cols-2">
                <Box>
                  <Typography variant="caption" className="text-muted-foreground">{t('columns.name')}</Typography>
                  <Typography variant="body1" className="font-medium">{toStr(item.name)}</Typography>
                </Box>
                <Box>
                  <Typography variant="caption" className="text-muted-foreground">{t('columns.pointsRequired')}</Typography>
                  <Typography variant="body1" className="font-medium">{item.points_required}</Typography>
                </Box>
                <Box>
                  <Typography variant="caption" className="text-muted-foreground">{t('columns.stock')}</Typography>
                  <Typography variant="body1" className="font-medium">{item.stock_quantity}</Typography>
                </Box>
                {item.exchanges_count != null && (
                  <Box>
                    <Typography variant="caption" className="text-muted-foreground">{t('form.giftExchangesCountLabel')}</Typography>
                    <Typography variant="body1" className="font-medium">{item.exchanges_count}</Typography>
                  </Box>
                )}
                {item.shop_product_variant_id != null && (
                  <Box>
                    <Typography variant="caption" className="text-muted-foreground">{t('form.giftShopVariantIdLabel')}</Typography>
                ...
                {item.description && (
                  <Box className="sm:col-span-2">
                    <Typography variant="caption" className="text-muted-foreground">{t('columns.description')}</Typography>
                    <Typography variant="body1" className="font-medium">{toStr(item.description)}</Typography>
                  </Box>
                )}
                {item.terms_conditions && (
                  <Box className="sm:col-span-2">
                    <Typography variant="caption" className="text-muted-foreground">{t('form.giftTermsConditions')}</Typography>
