 ...
 * bgGradient({
 *   images: [
 *     `linear-gradient(0deg, ${varAlpha(palette.primary.darkerChannel, 0.8)}, ${varAlpha(palette.primary.darkerChannel, 0.8)})`,
...
export function bgGradient({ sizes, repeats, images, positions }: BgGradientProps): React.CSSProperties {
  return {
 ...
 * // With image overlay
 * bgBlur({
 *   color: varAlpha(palette.background.paperChannel, 0.8),
...
export function bgBlur({ color, blur = 6, imgUrl }: BgBlurProps): React.CSSProperties & {
